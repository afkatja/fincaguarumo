...
// Query: *[_type == "post" && defined(slug.current)][0...12]{  _id, title, slug, mainImage, _createdAt, _updatedAt, isPublished}
...
// Query: *[_type == "page" && defined(slug.current)][0...12]{  _id, title, slug, subtitle, body, _createdAt, _updatedAt, isPublished}
export type ALL_PAGES_QUERYResult = Array<never>;
// Variable: PAGES_QUERY
// Query: *[_type == "page" && slug.current == $slug && language == $language][0] {  title, subtitle, description, mainImage, body, language, slug, isPublished, showBookingOptions, showBookingDialog,  slideshow->{images}, price,    "translations": *[      _type == "translation.metadata" &&       ^._id in translations[].value._ref    ][0].translations[]{      ...(value->{        language,        title,        subtitle,        mainImage,        slug,         body,        showBookingOptions,        showBookingDialog      })    }}
...
// Variable: PAGE_QUERY
// Query: *[_type == 'page' && slug.current == $pageName && language == $language][0] {    title, subtitle, description, mainImage, body     // {    //   ..., {    //     markDefs[] {    //     ...,    //     _type == "internalLink" => {    //       ...,    //       "slug": @.reference-> slug    //     }    //   }},    // }    , language, isPublished,    "translations": *[      _type == "translation.metadata" &&       ^._id in translations[].value._ref    ][0].translations[]{      ...(value->{        language,        title,        subtitle,        mainImage,        slug,         body, isPublished      })    }  }
...
// Query: *[_type == 'tour' && defined(slug.current) && language == $language]{  slug,  title,   mainImage,  description,   dateAdded,  language,  isPublished,  _createdAt,  _updatedAt,  "translations": *[      _type == "translation.metadata" &&       ^._id in translations[].value._ref    ][0].translations[]{      ...(value->{        language,        title,        slug, description      })    }}
...
// Query: *[_type == 'dialog' && _id == $dialogId][0] {  'cta': CTA_button,  'date': Date_label,  'selectDate': Select_date,  'guests': Guests_label,  'adults': Adults_label,  'adult': Adult_label,  'child': Child_label,  'other': Other_label,  'paymentMethod': Payment_method_label,  'creditCard': Credit_card_label,  'paypal': Paypal_label,  'people': People_label,  'person': Person_label,  'total': Total_label,  'ok': OK_button_label,  'cancel': Cancel_button_label,  "translations": *[      _type == "translation.metadata" &&       ^._id in translations[].value._ref    ][0].translations[]{      ...(value->)    }}
export type DIALOG_QUERYResult = null;
// Variable: TOUR_QUERY
// Query: *[_type == 'tour' && slug.current == $slug && language == $language][0]{  _id,   language,  title,   slug,   description,   mainImage, isPublished,  slideshow->{images},   price,   location,   geo,  duration,  body,  dialog,  "translations": *[      _type == "translation.metadata" &&       ^._id in translations[].value._ref    ][0].translations[]{      ...(value->{        language,        title,        slug,        description,        body,      })    }}
