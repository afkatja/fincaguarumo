...
// Query: *[_type == "page" && slug.current == $slug && language == $language][0] {  title, subtitle, description,   mainImage {    ...,     'metadata': asset->metadata  },   body, language, slug, isPublished, showBookingOptions, showBookingDialog,  slideshow->{  images[]{    ...,    'metadata': asset->metadata  }},  price, faq[]->{ question, answer, slug, keywords, showOnVillaBruno, category },  "translations": coalesce(    *[_type == "translation" && ^._id in translations[].value._ref][0].translations[]{      ...(value->{        language,        title,        subtitle,        description,        mainImage {..., 'metadata': asset->metadata},        slug,         body,        showBookingOptions,        showBookingDialog,        faq[]->{ question, answer, slug, keywords, showOnVillaBruno, category }      })    },    []  )}
...
// Variable: FAQ_QUERY
// Query: *[_type == 'faq' && language == $language] | order(displayOrder asc) {    category, question, answer, keywords, showOnVillaBruno, slug, language,    "translations": *[      _type == "translation.metadata" &&       ^._id in translations[].value._ref    ][0].translations[]{      ...(value->{        language,        category, question, answer, keywords, showOnVillaBruno, slug      })    }  }
