...
export const QuickInfoBar = ({ content, price, guests }: QuickInfoBarProps) => {
  ...
  return (
    <div className="rounded-lg p-4 md:p-6 border border-guarumo-primary/20 dark:border-none">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
        {/* Price */}
        <div className="flex flex-col items-center text-center p-3 md:p-4 bg-zinc-50/50 rounded-lg min-h-0">
          <DollarSign className="h-6 w-6 md:h-8 md:w-8 text-guarumo-primary dark:text-zinc-200 mb-1 md:mb-2 shrink-0" />
          <span className="text-xs md:text-sm text-gray-600 dark:text-zinc-200 leading-tight">
            {t("from")}
          </span>
          <span className="text-lg md:text-2xl font-bold text-guarumo-primary dark:text-zinc-50 leading-tight">
            {formatPrice(price)}
          </span>
          <span className="text-xs text-gray-500 dark:text-zinc-200  leading-tight">
            {b("priceFromShort", {
              guests: 1,
        ...
        {/* Capacity */}
        <div className="flex flex-col items-center text-center p-3 md:p-4 bg-zinc-50/50 rounded-lg min-h-0">
          <User2Icon className="h-6 w-6 md:h-8 md:w-8 text-guarumo-primary dark:text-zinc-200 mb-1 md:mb-2 flex-shrink-0" />
          <span className="text-xs md:text-sm text-gray-600 dark:text-zinc-200 leading-tight">
            {t("capacity")}
          </span>
          <span className="text-lg md:text-2xl font-bold text-guarumo-primary dark:text-zinc-50 leading-tight">
        ...
        {/* Property Type */}
        <div className="flex flex-col items-center text-center p-3 md:p-4 bg-zinc-50/50 rounded-lg min-h-0">
          <HomeIcon className="h-6 w-6 md:h-8 md:w-8 text-guarumo-primary dark:text-zinc-200 mb-1 md:mb-2 flex-shrink-0" />
          <span className="text-xs md:text-sm text-gray-600 dark:text-zinc-200 leading-tight">
            {t("type")}
          </span>
        ...
        <div className="flex flex-col items-center text-center p-3 md:p-4 bg-zinc-50/50 rounded-lg min-h-0">
          <Icon
            icon="Pin"
            className="h-6 w-6 md:h-8 md:w-8 text-guarumo-primary dark:text-zinc-200 mb-1 md:mb-2 flex-shrink-0"
          />
          <span className="text-xs md:text-sm text-gray-600 dark:text-zinc-200 leading-tight">
            {t("location")}
          </span>
      ...
      <div className="mt-3 md:mt-4 pt-3 md:pt-4 border-t border-guarumo-primary/20 flex flex-col sm:flex-row justify-center items-center gap-3 sm:gap-8 text-xs md:text-sm">
        <div className="flex items-center gap-2">
          <Icon
            icon="Clock"
            className="h-3 w-3 md:h-4 md:w-4 text-guarumo-primary dark:text-zinc-200 flex-shrink-0"
          />
          <span className="text-gray-600 dark:text-zinc-200 whitespace-nowrap">
            {t("checkIn")}:
          </span>
