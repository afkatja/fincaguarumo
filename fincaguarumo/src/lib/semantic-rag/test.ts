  ...
  try {
    ...
    try {
      ...
      if (embedding.dimensions !== 768) {
        console.error(
          `❌ Embedding generation failed: Wrong dimensions (${embedding.dimensions})`,
        )
        return false
      }

      console.log(
        `✅ Embedding generation passed! (${embedding.dimensions} dimensions)`,
      )
    } catch (error) {
    ...
    try {
      ...
      if (documents.length === 0) {
        console.warn(
          "⚠️  No documents found - this might be expected if Sanity is empty",
        )
      } else {
        console.log(
          `✅ Document processing passed! (${documents.length} documents)`,
        )
    ...
    try {
      ...
      if (results.length > 0) {
        ...
        if (
          !sampleResult.contentId ||
          !sampleResult.contentType ||
...
export async function runPerformanceTests() {
  ...
  try {
    ...
    await createAdaptiveRetrievalChain("test query for performance", {
      language: "en",
    })
