...
export function getSemanticRAGConfig(): SemanticRAGConfig {
  const vectorStore = (process.env.VECTOR_STORE || 'pgvector') as 'pgvector' | 'qdrant'
  const enableBinaryQuantization = process.env.ENABLE_BINARY_QUANTIZATION === 'true'
...
export function isQdrantConfigured(): boolean {
  ...
  return config.vectorStore === 'qdrant' && 
         !!config.qdrantUrl && 
