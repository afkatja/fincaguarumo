...
export async function migrateEmbeddingsToQdrant(
  ...
  const { contentType, language, batchSize = 50, dryRun = false } = options
  ...
  try {
    ...
    for (let i = 0; i < embeddings.length; i += batchSize) {
      ...
      console.log(
        `Processing batch ${batchNumber}/${totalBatches} (${batch.length} embeddings)`,
      )
      ...
      try {
        ...
        const qdrantEmbeddings = batch.map(embedding => ({
          ...
          language: embedding.language as any, // Cast to satisfy type constraint
