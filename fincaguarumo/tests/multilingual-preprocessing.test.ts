...
describe("Multilingual Preprocessing Integration Tests", () => {
  describe("Language Detection", () => {
    ...
    test("should detect Dutch text correctly", () => {
      const dutchText =
        "Hallo, hoe gaat het met je vandaag? Ik hoop dat het goed gaat."
    ...
    test("should detect Russian text correctly", () => {
      const russianText =
        "Привет, как дела сегодня? Надеюсь, у тебя всё хорошо."
    ...
    test("should detect German text correctly", () => {
      const germanText =
        "Hallo wie geht es dir heute Ich hoffe es geht dir gut der die und"
  ...
  describe("Text Preprocessing", () => {
    ...
    test("should preprocess Spanish text with character normalization", () => {
      const text = "No puedo hacerlo porque no tengo tiempo"
      const result = preprocessText(text, "es", { removeSpecialChars: true })
    ...
    test("should preprocess Russian text with character normalization", () => {
      ...
      const result = preprocessText(text, "ru", { removeSpecialChars: true })
    ...
    test("should preprocess German text with umlaut normalization", () => {
      ...
      const result = preprocessText(text, "de", { removeSpecialChars: true })
  ...
  describe("Preprocessing with Fallback", () => {
    test("should fallback to English for unsupported language", () => {
      ...
      expect(result.originalLanguage).toBe("en") // Fallback changes original language
      expect(result.detectedLanguage).toBe("en")
    })
  ...
  describe("Edge Cases", () => {
    test("should handle empty text gracefully", () => {
      const result = preprocessText("", "en", { minLength: 0 })

      expect(result.processedText).toBe("")
      expect(result.detectedLanguage).toBe("en") // Empty text defaults to specified language
    })
    ...
    test("should handle text with special characters", () => {
      const text =
        "Hello @world #test $money %percent &amp *star +plus -minus =equals"
      const result = preprocessText(text, "en", { removeSpecialChars: true })
    ...
    test("should handle text with mixed languages", () => {
      const mixedText = "Hello world how are you"
      const result = preprocessText(mixedText, "auto")

      expect(result.processedText).toBeTruthy()
      expect(result.detectedLanguage).toBe("en") // Should detect English keywords
    })

    test("should handle text with emojis", () => {
      const text =
        "Hello world! How are you? I'm fine! Thanks! Have a great day!"
  ...
  describe("Integration with Other Components", () => {
    test("should work correctly with embedding generation pipeline", () => {
      ...
      expect(result.processedText).toBe(
        "is test integration embedding generation",
      )
      expect(result.processedText.split(" ").length).toBeLessThan(
        text.split(" ").length,
      )
    })
