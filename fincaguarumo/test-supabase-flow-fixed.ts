...
/**
 * Test script for Supabase booking flow
 * This simulates the exact data structure and operations from the Stripe webhook
 * Run with: npx ts-node test-supabase-flow-fixed.ts
 */
...
// Simple date parsing function (replicating parsePropertyDate logic)
function parsePropertyDate(dateString: string): Date {
...
const testEventId = "cs_test_fixed_" + Date.now()

// Replicate the exact booking details structure from webhook
const bookingDetails = {
  type: testMetadata.type || "villa",
...
async function testFixedSupabaseFlow() {
  console.log("🧪 Starting FIXED Supabase flow test...")
  ...
  try {
    ...
    // Save to Supabase with all required fields (FIXED: added .select())
...
// Run the test
testFixedSupabaseFlow().catch(console.error)
